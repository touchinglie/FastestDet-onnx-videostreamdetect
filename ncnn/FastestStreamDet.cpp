...
void GetFileRealName(char *path, char *filename)
{
    char *ptr = strtok(path, ".");
    strcpy(filename, ptr);
}
...
cv::Mat MainRun(cv::Mat inputPic)
{
    cv::Mat img = inputPic;
    ...
    std::vector<TargetBox> empty_box;
...
int main(int argc, char *argv[])
{
    ...
    GetFileRealName(fileName, fileRealName);
    ...
    if (!strcmp(kind, pickind))
    {
        ...
        cv::Mat resultpic = MainRun(img);
        char outPath[] = "./output/";
        strcat(fileRealName, "_result.jpg");
        strcat(outPath, fileRealName);
        cv::imwrite(outPath, resultpic);
    }
    else
    {
        ...
        int Decoder = cv::VideoWriter::fourcc('M', 'J', 'P', 'G');
        vwriter.open(savePath, Decoder, rate, frame.size(), true);
        if (!vwriter.isOpened())
        {
            printf("StreamWriter load failed.\n");

            return 1;
        }
        printf("StreamWriter load succeed.\n");
